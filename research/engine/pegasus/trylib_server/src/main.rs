...
use pegasus_server::job::{DyLibParser, JobDesc};
use prost::Message;
...
struct Config {
    ...
    config_dir: PathBuf,
    #[structopt(long = "lib", parse(from_os_str))]
    lib_dir : PathBuf,
...
async fn main() {
    ...
    if config.server {
        pegasus_server::cluster::standalone::start(rpc_config, server_config, DyLibParser)
            .await
        ...
        client.add_library("libtrylib", config.lib_dir).await.expect("add library failure");

        let start = Instant::now();
        for i in 0..config.times {
            let mut conf = JobConf::with_id(i + 1, "TryLib_Echo_Example", 1);
            conf.reset_servers(ServerConf::All);
            let mut job_desc = JobDesc::default();
            job_desc.set_input(vec![8u8; 8])
                .set_plan(b"build_job".to_vec())
                .set_resource(b"libtrylib".to_vec());
